...
export const sha256Base64Url = (bytes: Uint8Array<ArrayBuffer>) =>
...
export const parseFailedUpdateIds = (header: string | undefined): ReadonlyArray<string> =>
  header === undefined ? [] : [...header.matchAll(/"([^"]*)"/g)].map((match) => match[1]!.toLowerCase());
...
export const decide = Effect.fn("Protocol.decide")(function* (
  ...
  if (newest.rolloutPercent < 100) {
    ...
    const bucket = clientId === undefined ? 100 : yield* rolloutBucket(clientId, newest.id);
  ...
  if (chosen.kind === "rollback") {
    return current === headers["expo-embedded-update-id"]?.toLowerCase()
      ? none
