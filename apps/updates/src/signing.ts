...
export interface SignerShape {
  ...
  readonly sign: (bytes: Uint8Array<ArrayBuffer>) => Effect.Effect<string>;
}
...
export class Signer extends Context.Service<Signer, SignerShape>()("expo-ota/Signer") {
  static readonly fromKey = (key: CryptoKey, keyId: string) =>
    Layer.succeed(Signer, {
      keyId,
      sign: (bytes) =>
        Effect.promise(() => crypto.subtle.sign(algorithm, key, bytes)).pipe(Effect.map(base64)),
    });
    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        const key = yield* Effect.promise(() =>
          crypto.subtle.importKey("pkcs8", pkcs8FromPem(pem), algorithm, false, ["sign"]),
        );
        return { keyId, sign: (bytes) => Effect.promise(() => crypto.subtle.sign(algorithm, key, bytes)).pipe(Effect.map(base64)) };
      }),
    );
}
