...
const BundlesQuery = Schema.Struct({
  ...
  limit: Schema.optional(Schema.NumberFromString.check(Schema.isBetween({ minimum: 1, maximum: 10 }))),
});
...
export const routes = HttpRouter.use(
  Effect.fn("Updates.routes")(function* (router) {
    ...
    const afterResponse = <E>(work: Effect.Effect<void, E, RuntimeContext>) =>
      Effect.gen(function* () {
        const execution = yield* Cloudflare.Workers.WorkerExecutionContext;
        yield* execution.waitUntil(work.pipe(Effect.catch((error) => Effect.logWarning("Bookkeeping failed", { cause: error }))));
      });
    ...
    const manifest = handle(
          ...
          Effect.all(
            [
              clientId === undefined || failedUpdateIds.length === 0
                ? Effect.void
                : store.recordFailures({ clientId, updateIds: failedUpdateIds, fatalError: headers["expo-fatal-error"] }),
              clientId === undefined
                ? Effect.void
                : store.recordCheck({
                    clientId,
      ...
      }).pipe(Effect.withSpan("Updates.manifest")),
    );
    ...
    const patchFor = Effect.fn("Updates.patchFor")(function* (headers: Headers.Headers, hash: string) {
      const offered = headers["a-im"]?.split(",").some((token) => token.trim().toLowerCase() === "bsdiff") ?? false;
    ...
    const asset = handle(
      Effect.gen(function* () {
        ...
        const patch = yield* patchFor(request.headers, hash);
        if (patch !== null) {
      ...
      }).pipe(Effect.withSpan("Updates.asset")),
    );
    ...
    const missingAssets = handle(
      authorized(
        ...
        }).pipe(Effect.withSpan("Updates.missingAssets")),
      ),
    );
    ...
    const putAsset = handle(
      authorized(
        ...
        }).pipe(Effect.withSpan("Updates.putAsset")),
      ),
    );
    ...
    const branchBundles = handle(
      authorized(
        ...
        }).pipe(Effect.withSpan("Updates.branchBundles")),
      ),
    );
    ...
    const putPatch = handle(
      authorized(
        ...
        }).pipe(Effect.withSpan("Updates.putPatch")),
      ),
    );
    ...
    const publishGroup = handle(
      authorized(
        Effect.gen(function* () {
          ...
          const platformUpdates = [input.updates.ios, input.updates.android].filter((u) => u !== undefined);
          if (platformUpdates.length === 0) {
            return yield* Effect.fail(new BadRequest({ message: "At least one platform update is required." }));
          }
          const referenced = platformUpdates.flatMap((update) =>
            "rollbackToEmbedded" in update ? [] : [update.launchAsset.hash, ...update.assets.map((a) => a.hash)],
          );
          const missing = yield* store.missingAssets([...new Set(referenced)]);
          if (missing.length > 0) {
            return yield* Effect.fail(new BadRequest({ message: `Assets not uploaded: ${missing.join(", ")}` }));
          }
          const group = yield* store.publishGroup(input);
          return HttpServerResponse.jsonUnsafe(group, { status: 201 });
        }).pipe(Effect.withSpan("Updates.publishGroup")),
      ),
    );
