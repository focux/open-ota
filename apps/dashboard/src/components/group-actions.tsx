...
export function GroupActions({
  ...
  const reach = group.updates.map((update) => ({
    update,
    devices: deviceCount(metrics, update.platform, update.runtimeVersion),
  }))
      ...
      >
        <DialogContent className="sm:max-w-lg">
          ...
          {unreachable.length > 0 && (
            <Alert>
              <AlertTitle>This reaches nobody yet</AlertTitle>
              <AlertDescription className="text-pretty">
                ...
                {unreachable
                  ...
                  .join(" or ")}{" "}
                yet, so this update reaches nobody until one ships.
              </AlertDescription>
            </Alert>
          )}
